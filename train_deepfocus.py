...
def parse_option():
    parser = argparse.ArgumentParser(
        'Thesis cnunezf training LGFFEM model over DeepFocus using SubCenterArcFaceLoss.', add_help=True)
    ...
    parser.add_argument('--dataset_path',
                        type=str,
                        default='/thesis/classical/deepfocus', 
...
#     if not torch.cuda.is_available(): raise Exception('This script is only available to run in GPU.')
    ...
    # Load the dataset
    print(f'[+] Loading DeepFocus dataset...')
    ...
    training_loader = deepfocus.train_dataloader
    print('[+] Ready !')
    ...
    ## Prepare Loss Metric
    loss_func = losses.SubCenterArcFaceLoss(num_classes=10, embedding_size=4*base_config.MODEL.NECK.NUM_CHANNELS, margin=args.loss_m, scale=args.loss_m, sub_centers=args.loss_sc).to(device)
    ...
    for e, epoch in enumerate(range(start_epoch, end_epoch + 1)):
        ...
        with tqdm(training_loader, unit=" batch") as tepoch:
            for batch_idx, data in enumerate(tepoch):
                ...
                images = train_transforms(data[0].permute(0, 3, 1, 2)).to(device) ## TEMPORAL CHANGED; TODO: REWRITE DATASET WITH TRANSFORM OPTION
                labels = torch.tensor(data[1].numpy(), dtype=torch.int64).to(device)
