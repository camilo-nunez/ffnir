...
def parse_option():
    parser = argparse.ArgumentParser(
        'Thesis cnunezf training LGFFEM model over PanNuke using SubCenterArcFaceLoss.', add_help=True)
    ...
    parser.add_argument('--dataset_path',
                        type=str,
                        default='/thesis/classical/pannuke', 
...
if __name__ == '__main__':
    ...
    # Load the dataset
    print(f'[+] Loading PanNuke dataset...')
    print(f'[++] Using batch_size: {args.batch_size}')
    n_classes_pannuke = 6
    hover_transform = A.Compose([A.VerticalFlip(p=0.5), 
    ...
    pannuke = PanNukeDataModule(data_dir=args.dataset_path, 
                                download=False,
    ...
    training_loader = pannuke.train_dataloader
    print('[+] Ready !')
    ...
    ## Prepare Loss Metric
    loss_func = losses.SubCenterArcFaceLoss(num_classes=len(TISSUE_TYPES), embedding_size=4*base_config.MODEL.NECK.NUM_CHANNELS, margin=args.loss_m, scale=args.loss_m, sub_centers=args.loss_sc).to(device)
    ...
    for e, epoch in enumerate(range(start_epoch, end_epoch + 1)):
        ...
        with tqdm(training_loader, unit=" batch") as tepoch:
            for batch_idx, data in enumerate(tepoch):
                ...
                labels = torch.tensor([TISSUE_TYPES[i] for i in data[3]], dtype=torch.int64)
        ...
        if loss_mean < best_loss:
            ...
            torch.save({'model_neck_state_dict': base_model.neck.state_dict(),
                       ...
                       os.path.join(args.checkpoint_path, f'{datetime.utcnow().strftime("%Y%m%d_%H%M")}-EMB-PAN-{Path(args.cfg_model_backbone).stem}-{Path(args.cfg_model_neck).stem}-{Path(args.cfg_model_head).stem}-epoch{epoch}.pth'))
